...
| Item | Location | Notes |
|---|---|---|
| The 7 stock photographs (testimonial photo/logos excluded) | `assets/img/placeholder-*.jpg` | AI-generated Stitch placeholders (control room, server rack, engineer at screen, board review, collaboration office, factory floor, logistics warehouse). Real photos/screenshots should replace them 1:1 (same filenames, or update the `src`/`background-image` references in `src/pages/*.html`). |
| Testimonials | `src/pages/index.html`, `src/pages/en/index.html` (section 4.6) | Real content: André Fernandes (CEO, Suffa, with photo and Suffa logo) and Wallace Henrique Trevisol (Engenheiro de Processo, BITZER Compressores, Brazil; no profile photo, initials avatar intentional, BITZER logo). Both authorised quotes, names, roles, photo and logos on 2026-09-08 (confirmed by Eduardo). Not a placeholder. |
| Contact form backend | Web3Forms access key, injected at build time | The key is never committed. In CI it comes from the repository secret `WEB3FORMS_KEY` (Settings > Secrets and variables > Actions), passed to the build step as an environment variable. Locally, put it in a `web3forms.key` file at the repo root (git-ignored) or export `WEB3FORMS_KEY`. `build.js` writes it into `public/assets/js/main.js`; the source file keeps the constant empty and the build fails if that changes. Note: like any client-side form service key, it is visible in the published JavaScript on fluxforge.pt; keeping it out of the repository prevents it from being indexed with the source, and the Web3Forms dashboard settings (domain restriction, hCaptcha) limit abuse. |
...
| Privacy policy controller details | `src/pages/legal/politica-de-privacidade.html`, `src/pages/en/legal/privacy-policy.html`, section 1 | Company legal name, NIF and address intentionally omitted for now (Eduardo will supply them); add them back as list items under section 1 before launch. |
